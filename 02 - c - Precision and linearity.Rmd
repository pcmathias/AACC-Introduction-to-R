...
# Method Validation -- Precision, Calibration Verification, and Linearity
...
g <- g + geom_histogram(binwidth=_________)
g <- g + geom_vline(aes(xintercept = mean(P1) + 2 * sd(P1)), linetype=2, color="blue")
g <- g + geom_vline(aes(xintercept = _________), linetype=2, color="blue")
g
```
...
g <- g + geom_hline(_______)
g <- g + geom_hline(_______________)
g <- g + ylim(_________)
g
...
g <- g + ylab("Observed - Assigned")       # Change y-axis label
g
```
...
g <- g + geom_point()
g <- g + geom_hline(yintercept = 0, linetype=1, color="gray")
g <- g + ylim(____________)
g <- g + ylab(_________________)
g
```
