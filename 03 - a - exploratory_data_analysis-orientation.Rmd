...
```{r}
ggplot(data = orders) + geom_bar(mapping = aes(x = order_time))
```
...
```{r}
ggplot(data = orders) + geom_histogram(mapping = aes(x = order_time))
```
...
How do we fix this? The ggplot2 functions use seconds as their default unit of time, so we can define a binwidth that is multiples of seconds to reflect days or weeks. For example, one day = 60*60*24 seconds.

```{r}
ggplot(data = orders) + geom_histogram(mapping = aes(x = order_time), binwidth = 60*60*24)
```
...
```{r}
ggplot(data = orders) + geom_histogram(mapping = aes(x = order_time), binwidth = 60*60*24*7)
```
