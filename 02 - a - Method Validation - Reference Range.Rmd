...
To visually inspect, let's graphically show the bounds of the reference intervals on the histogram we made above.
...
This intermediate data structure `mask` is convenient, because we can easily ask how many elements it contains and how many are `TRUE`. The `length` function counts how many elements there are in an object:
```{r}
length(mask)
sum(mask == TRUE)
...
# Now write the same code for the log-transformed variables you calculated above
```
...
Are there outliers? How can you remove them? Here we use the function `filter` that uses boolean logic to specify which observations to keep.
