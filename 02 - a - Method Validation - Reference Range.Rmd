...
Relatively "clean" data has already been organized in separate tabs in an excel document. Let's first load one of these data sets. To do this we will use the `read_excel` function. We need to specify the path to our excel file and the worksheet we would like to load in. Below is an example, which does the following: 
 ...
 3. The result is *assigned* via the `<-` operation (also known as the assignment operator) to the variable to the left of the assignement operator.

Here's a graphical representation of how a function such as `read_excel` can assign values to a new variable:
...
*What is a path? Paths describe the location of a file in a filesystem (e.g., your computer, a network drive, etc.). In this case, we are telling the function `read_excel` the location of our file, and the name of the Excel Sheet to open. These are called "arguments" to the function. Functions "take" arguments (or alternatively functions are "passed" arguments).*
...
```{r}
cv <- sd(data$method_a) / mean(data$method_a) * 100  # Coefficient of variation
print(cv)
```
...
> *Exercise*
> Make a second histogram plotting `method_b` values.

``` {r}
g <-  ggplot(data)
g <- g + geom_histogram(bins=50, aes(x=method_b))
g
```
...
> **Exercise:** Calculate the parametric reference range assuming that the results are log-normal (rather than normally distributed). Hint: First transform the data as above with the `log()` function, and then "untransform" back into real numbers with `exp()`
...
2. Start with with the code for the histogram above, add additional lines of code to display the bounds of the parametric reference interval. (*Hint: Remember that ggplot allows you to "add" new elements to a plot*)
...
n_inrange <- with(data,              # Note: `with` allows you to specify columns below without writing `data` over and over again
